...
// --- 豆瓣推荐相关变量 (使用工作示例中的变量名) ---
let doubanPageStart = 0;
let doubanPageSize = 12; // 你可以调整每页显示的数量
let doubanCurrentTag = localStorage.getItem('doubanCurrentTag') || '热门'; // 默认加载热门
const doubanTags = ['热门', '美剧', '英剧', '国产剧', '日本动画', '纪录片', '综艺']; // 标签顺序
const DOUBAN_PROXY = "https://api.allorigins.win/raw?url="; // 沿用之前的代理
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- 初始化豆瓣推荐 ---
    renderDoubanTags(); // 创建标签按钮
    setupDoubanRefreshBtn(); // 设置换一批按钮监听
    renderRecommend(doubanCurrentTag, doubanPageSize, 0); // 初始加载默认分类第一页
    ...
    if (!localStorage.getItem('hasInitializedDefaults')) {
        ...
        // Re-apply default selection to UI
        initAPICheckboxes();
        updateSelectedApiCount();
    }
...
function initAPICheckboxes() {
    ...
    // Add Adult APIs section if applicable
    ...
    checkAdultAPIsSelected(); // Initial check
}
...
function addCustomApi() {
    ...
    if (exists) { showToast('已存在相同名称或链接的自定义API', 'warning'); return; }
...
function removeCustomApi(index) {
    ...
    selectedAPIs.forEach(id => {
        if (id === customApiIdToRemove) return;
...
function renderDoubanTags() {
    ...
    doubanTags.forEach(tag => {
        ...
        // 使用 text-xs 或 text-sm 控制大小
        btn.className = `douban-tag px-3 py-1 rounded text-xs font-medium mr-1 mb-1 transition ${tag === doubanCurrentTag ? 'bg-pink-600 text-white border-pink-600' : 'bg-[#222] text-gray-300 border border-transparent hover:bg-pink-700 hover:text-white'}`;
        btn.textContent = tag;
        btn.onclick = function() {
            if (doubanCurrentTag !== tag) {
                doubanCurrentTag = tag;
                localStorage.setItem('doubanCurrentTag', tag); // 保存当前标签
                doubanPageStart = 0; // 切换标签时重置页码
                renderRecommend(doubanCurrentTag, doubanPageSize, doubanPageStart); // 加载新标签数据
                renderDoubanTags(); // 重新渲染标签以更新高亮状态
            }
        };
...
function setupDoubanRefreshBtn() {
    ...
    btn.onclick = function() {
        ...
        // 简单的循环逻辑，可以根据需要调整（例如，如果知道总数，可以判断是否最后一页）
        // 豆瓣接口似乎没有明确的总数，所以简单循环
        if (doubanPageStart >= 90) { // 假设最多加载 3 页左右 (30*3=90)
            doubanPageStart = 0;
            showToast('已重新从第一页加载', 'info');
        }
...
function renderRecommend(tag, pageLimit, pageStart) {
    ...
    container.appendChild(loading); // 把加载指示器放进去
    if (refreshBtn) refreshBtn.disabled = true; // 禁用换一批按钮
    ...
    const url = DOUBAN_PROXY + encodeURIComponent(target); // 使用豆瓣代理

    fetch(url)
        .then(response => {
            if (!response.ok) {
                throw new Error(`豆瓣 API 响应错误: ${response.status}`);
            }
            return response.json();
        })
        .then(data => {
            loading.style.display = 'none'; // 隐藏加载指示器
            container.innerHTML = ''; // 再次清空，准备添加卡片

            if (!data || !data.subjects || data.subjects.length === 0) {
                container.innerHTML = `<div class="col-span-full text-gray-400 text-center py-10">该分类下没有更多推荐了。</div>`;
                if (refreshBtn) {
                    refreshBtn.disabled = true; // 没有数据了，禁用按钮
            ...
            data.subjects.forEach(tv => {
                ...
                item.innerHTML = `
                    <button
                      class="relative w-full flex justify-center group active:scale-95 transition transform duration-150 ease-in-out mb-2"
                      onclick="fillAndSearch('${tv.title.replace(/'/g, "\\'")}')"
                      title="搜索 ${tv.title}"
                    >
                        <div class="relative w-full aspect-[2/3] overflow-hidden rounded-md bg-black/10"> {/* 容器定比例 */}
                            <img
                                src="${proxyImage(tv.cover)}"
                                alt="${tv.title}"
                                class="w-full h-full object-contain" /* 图片 contain */
                                loading="lazy"
                                onerror="this.onerror=null; this.src='https://via.placeholder.com/200x300?text=Image+Error'; this.classList.add('object-contain');"
                            />
                            <div class="absolute bottom-1 left-1 bg-black/70 text-white text-xs font-semibold px-1.5 py-0.5 rounded">
                                评分: ${tv.rate || "N/A"}
                            </div>
                        </div>
                    </button>
                    <a
                        ...
                        class="w-full block text-sm font-semibold truncate hover:text-cyan-400 mt-1" /* 调整链接样式 */
                        title="${tv.title}"
                    >
                        ${tv.title}
                    </a>
            ...
            // 如果返回的数据少于请求的数量，也认为是最后一页
            if (refreshBtn) {
                 if (data.subjects.length < pageLimit) {
                    refreshBtn.disabled = true;
                    refreshBtn.textContent = '没有更多了';
                 } else {
                    refreshBtn.disabled = false; // 恢复按钮
        ...
        .catch(err => {
            loading.style.display = 'none'; // 隐藏加载
            container.innerHTML = `<div class="col-span-full text-red-400 text-center py-10">❌ 获取推荐数据失败，请稍后重试或检查网络。(${err.message})</div>`;
            console.error(`豆瓣 API 请求失败 (Tag: ${tag}, Start: ${pageStart}):`, err);
            if (refreshBtn) {
                refreshBtn.disabled = true; // 出错也禁用按钮
...
function proxyImage(originalUrl) {
    ...
    // 简单的 Referer 绕过代理示例 (需要服务器支持)
    // return `https://images.weserv.nl/?url=${encodeURIComponent(originalUrl.replace(/^https?:\/\//, ''))}&referer=movie.douban.com`;
    return originalUrl; // 暂时返回原始 URL
}

// 点击推荐卡片填充搜索框并搜索
function fillAndSearch(title) {
    const searchInput = document.getElementById('searchInput');
    const doubanSection = document.getElementById("doubanSection"); // 使用外层 ID
    const resultsArea = document.getElementById("resultsArea");
    const searchArea = document.getElementById('searchArea');

    if (searchInput) {
        searchInput.value = title;
        search(); // Call existing search function

        // Hide Douban section, show results section
        if (doubanSection) doubanSection.style.display = "none";
        if (resultsArea) resultsArea.classList.remove('hidden');
        // Adjust search area layout
        if (searchArea) {
            // searchArea.classList.remove('flex-1'); // 可能不再需要 flex-1
...
async function search() {
    ...
    if (window.isPasswordProtected && window.isPasswordVerified) {
        if (window.isPasswordProtected() && !window.isPasswordVerified()) {
            showPasswordModal && showPasswordModal(); return;
        }
    }
    ...
    try {
        ...
        const searchPromises = selectedAPIs.map(async (apiId) => {
             try {
                ...
                if (!data || !data.list || !Array.isArray(data.list)) {
                     if (data && data.code && ![0, 1, 200].includes(data.code)) return [];
                     if (!data || typeof data !== 'object' || (data.list && !Array.isArray(data.list))) return [];
                }
                ...
                return (data.list || []).map(item => ({
                    ...
                    source_name: apiName,
                    source_code: apiId,
                    api_url: apiId.startsWith('custom_') ? sourceUrl : undefined
                }));
            } catch (error) {
                if (error.name !== 'AbortError') console.warn(`API ${apiId} search failed:`, error);
                else console.warn(`API ${apiId} search timed out.`);
                return [];
            }
        });
        ...
        // Handle no results & filtering
        if (allResults.length === 0) { /* ... show no results ... */ hideLoading(); return; }
        const yellowFilterEnabled = localStorage.getItem('yellowFilterEnabled') === 'true';
        if (yellowFilterEnabled) { /* ... filter logic ... */ }
        if (allResults.length === 0) { /* ... show filtered message ... */ hideLoading(); return; }

        // Render search result cards (Using object-cover as before)
            ...
            return `
                <div class="card-hover bg-[#1a1a1a] rounded-lg overflow-hidden cursor-pointer transition-all hover:scale-[1.02] hover:shadow-lg h-full flex flex-col"
                    ...
                    <div class="flex-shrink-0">
                        ${hasCover ? `
                        <div class="relative overflow-hidden aspect-[2/3] bg-black/10">
                             <img src="${proxyImage(item.vod_pic)}" alt="${safeName}"
                                  class="w-full h-full object-cover transition-transform group-hover:scale-105"
                                  onerror="this.onerror=null; this.src='https://via.placeholder.com/200x300?text=Error'; this.classList.add('object-contain');"
                                  loading="lazy">
                             <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-80 pointer-events-none"></div>
                        </div>` :
                        `<div class="h-48 flex items-center justify-center bg-[#2a2a2a] text-gray-500 aspect-[2/3]">无封面</div>`}
                    </div>
        ...
        hideLoading();
        if (doubanSection && doubanSection.style.display !== "none") doubanSection.style.display = "none"; // Ensure hidden on error too
    }
}
...
async function showDetails(id, vod_name, sourceCode) {
    ...
    try {
        ...
        let sourceNameForModal = '未知来源'; // Default

        if (sourceCode.startsWith('custom_')) {
            const customIndex = sourceCode.replace('custom_', '');
            const customApi = getCustomApiInfo(customIndex);
            if (!customApi) { throw new Error('无法找到自定义API配置'); }
            apiUrlForDetails = customApi.url + API_CONFIG.detail.path;
            detailParams += `&customApi=${encodeURIComponent(customApi.url)}&source=custom`;
            sourceNameForModal = customApi.name; // Use custom name
        } else {
            if (!API_SITES[sourceCode]) { throw new Error('无效的内置API源'); }
            apiUrlForDetails = API_SITES[sourceCode].api + API_CONFIG.detail.path;
            detailParams += `&source=${sourceCode}`;
            sourceNameForModal = API_SITES[sourceCode].name; // Use built-in name
        }

        const fullDetailApiUrl = apiUrlForDetails + id;
        ...
        const videoInfo = data.videoInfo || (data.list && data.list[0]) || {};
        const episodesRaw = data.episodes || videoInfo.vod_play_url || '';
        const displayTitle = vod_name || videoInfo.vod_name || '未知视频';

        // Use determined source name
        modalTitle.innerHTML = `<span class="break-words">${displayTitle}</span><span class="text-sm font-normal text-gray-400 ml-2">(${sourceNameForModal})</span>`;
        ...
        if (typeof episodesRaw === 'string' && episodesRaw.includes('$')) {
            ...
            if (playSources.length > 0) {
                ...
                }).filter(url => url && (url.startsWith('http://') || url.startsWith('https://') || url.startsWith(PROXY_URL)));
            }
...
function playVideo(url, vod_name, episodeIndex = 0) {
    ...
    if (modalTitle) {
        ...
        if (sourceSpan) { const match = sourceSpan.textContent.match(/\(([^)]+)\)/); if (match && match[1]) { sourceName = match[1].trim(); } }
    }
    ...
    const videoInfo = {
        title: videoTitle, url: url, episodeIndex: episodeIndex, sourceName: sourceName, timestamp: Date.now(),
...
function playPreviousEpisode() {
     if (currentEpisodeIndex > 0) {
        ...
        // Ensure episode URL exists at the previous index
        if (currentEpisodes && currentEpisodes[prevIndex]) {
...
function playNextEpisode() {
     if (currentEpisodes && currentEpisodeIndex < currentEpisodes.length - 1) {
        const nextIndex = currentEpisodeIndex + 1;
         // Ensure episode URL exists at the next index
        if (currentEpisodes[nextIndex]) {
...
function renderEpisodes(vodName) {
    ...
    return episodes.map((episode, index) => {
        ...
        if (!episode || typeof episode !== 'string') return ''; // Skip invalid episode data
...
function setupEventListeners() {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (settingsPanel && settingsButton && !settingsPanel.contains(e.target) && !settingsButton.contains(e.target) && settingsPanel.classList.contains('show')) { toggleSettings(); } // Use toggle function
        if (historyPanel && historyButton && !historyPanel.contains(e.target) && !historyButton.contains(e.target) && historyPanel.classList.contains('show')) { toggleHistory(); } // Use toggle function
    });
    const yellowFilterToggle = document.getElementById('yellowFilterToggle');
    if (yellowFilterToggle) { yellowFilterToggle.addEventListener('change', function(e) { localStorage.setItem('yellowFilterEnabled', e.target.checked); }); }
    const adFilterToggle = document.getElementById('adFilterToggle');
    if (adFilterToggle) { adFilterToggle.addEventListener('change', function(e) { localStorage.setItem(PLAYER_CONFIG.adFilteringStorage, e.target.checked); }); }
}
...
function resetSearchArea() {
    ...
    if (doubanSection) doubanSection.style.display = "block"; // Restore Douban section
